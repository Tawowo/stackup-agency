      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 pt-28 pb-20 w-full">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <h1 aria-label="Le site web qui fait décoller votre activité."
              ...
              {words.map((word, i) => (
                <span key={i} className="word-mask" style={{ marginRight: '0.28em' }}>
                  <span className="word-inner" style={{ transitionDelay: `${i * 60}ms` }}>{word}</span>
                </span>
              ))}
            </h1>

            <p className="text-white/70 text-lg lg:text-xl mb-6 max-w-xl leading-relaxed">
            ...
            <div className="flex flex-wrap gap-4 mb-5">
              ...
              <Link href="/services"
                className="px-7 py-4 border border-white/20 text-white hover:bg-white/10 font-semibold rounded-xl transition-all">
          ...
          <div className="hidden lg:flex justify-center">
            <div className="flip-scene w-72">
              <div className={`flip-card w-full ${flipped ? 'flipped' : ''}`} style={{ height: '208px' }}>
                {/* Front: code */}
                <div className="flip-face w-full h-full rounded-2xl overflow-hidden border border-white/10 bg-[#0D1626] shadow-2xl">
                  ...
                  <div className="p-4 font-mono text-sm space-y-2">
                    {CODE_LINES.map((line, i) => (
                      <div key={i} className={`transition-all duration-300 ${i < typedLines ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-16">
          {[
            ...
            <div key={s.label} className="text-center p-4 rounded-xl bg-white/5 border border-white/10 reveal-item">
