...
const shapes = [
  { size: 380, x: '82%', y: '8%',  color: '#1E3A5F', opacity: 0.12, duration: 20 },
  { size: 260, x: '68%', y: '65%', color: '#F59E0B', opacity: 0.15, duration: 15 },
  { size: 180, x: '4%',  y: '72%', color: '#2D7DD2', opacity: 0.10, duration: 25 },
  { size: 130, x: '18%', y: '12%', color: '#F59E0B', opacity: 0.12, duration: 18 },
]
...
function CodeWindow() {
  ...
  return (
    <div className="hidden lg:block relative">
      <div className="bg-[#1E3A5F] rounded-xl border border-[#2D4F7A] overflow-hidden shadow-2xl shadow-[#1E3A5F]/20">
      ...
      <div className="absolute -inset-4 bg-[#2D7DD2]/10 rounded-2xl blur-xl -z-10" />
    </div>
  )
}
...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-gradient-to-br from-[#F0F4FF] to-[#FFF8F0]">
      {/* Floating shapes */}
      {shapes.map((s, i) => (
        <div
          key={i}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full">
        {/* Left */}
        <div>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1E3A5F]/10 border border-[#1E3A5F]/20 text-[#1E3A5F] text-sm font-medium mb-8">
            <span className="w-2 h-2 rounded-full bg-[#F59E0B] animate-pulse flex-shrink-0" />
            Agence digitale — France
          </div>

          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold text-[#0F172A] leading-tight mb-6">
            Votre vision.<br />
          ...
          <p className="text-base sm:text-lg lg:text-xl text-[#475569] mb-10 leading-relaxed max-w-lg">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            {[
              ...
              <div key={stat.label} className="text-center p-3 sm:p-4 rounded-xl bg-white border border-[#E2E8F0] shadow-sm">
                <div className="text-xl sm:text-2xl font-bold text-[#0F172A] mb-0.5">{stat.value}</div>
